...
- `requests.rs` — **Requests: form/document submissions routed through approvals**, AI-first. A request is a thin overlay (`requests` table, migration `0022`) that anchors to a *document* (an attached file, or one generated from the form) and **reuses the `file_workflows` engine** for routing — so approvals/notifications/turn-guards/send-back/audit are all shared, not duplicated (`p1::start_workflow_core` is the extracted shared entry). Forms are **admin-authored** (`request_forms`, migration `0023`): each request type has fields + a linked `workflow_templates` route; the four defaults are seeded non-destructively at startup (`bootstrap_request_forms`, called from `state.rs`) so agency edits survive reboots. Endpoints: `POST /api/requests/intake` (AI: plain language → `{kind,fields,title,summary,route}`), `POST /api/requests` (create + start workflow; **any signed-in user may submit**, approvals stay editor+), `GET /api/requests?box=mine|inbox|all`, `GET /api/requests/:id`, `POST /api/requests/:id/cancel` (withdraw, migration `0024`), and the **Form Designer CRUD** `GET/POST /api/forms` + `PATCH/DELETE /api/forms/:id` (editor+). Anchor docs live in a shared `sys_requests` area so every reviewer can read them. Decisions reuse `/api/workflow-steps/:id/decision` + `/send-back`; request notifications link to `/requests/:id` via `p1::subject_link`.
...
- **Everyday** (consumer UI, viewers/editors) — the `app/(everyday)/` route group (URL-transparent): a clean top-bar shell over `/home`, `/my`, `/shared`, `/recent`, `/starred`, `/requests`, and a friendly `/f/[id]` file view. Plain language (Systems → **"Areas"**), preview-first, minimal columns, no ETag/quota/rotation vocabulary. On phones the top nav becomes a bottom tab bar; `MobileUploadFab` hides on everyday routes (the bottom nav has its own Upload). **Requests** (`/requests`, `/requests/new`, `/requests/[id]`) is the AI-first form/document approval flow — chat-first intake or a manual form, both consuming the admin-authored forms from `GET /api/request-forms`; `components/everyday/request-*.tsx` + `request-bits.tsx`. The **Form Designer** (`/settings/forms`) and **Workflow Designer** (`/settings/workflows`) are Admin-Console-only — Everyday never shows them; both open their builder in a shared `components/modal.tsx` popup, are bilingual (EN/TH), and support create + edit.

`lib/view-mode.tsx` holds the mode (`fh-view` cookie, read server-side in `app/layout.tsx`). **Everyday is the default for every role** — admins included land on `/home` and step into the Workspace via the user menu's "Admin console" entry (`app/page.tsx` redirects `/` to `/home` unless the cookie explicitly says `workspace`). Everyday components live in `components/everyday/*` and reuse the workspace `FilePreview`/`FileAiPanel`. Keep the two shells' chrome distinct but the data layer shared.
