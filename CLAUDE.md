...
- `app/` — App Router pages. Workspace routes map 1:1 to the README screens (`/`, `/files` + layout variants `board`/`gallery`/`calendar`/`timeline`, `/files/[id]`, `/upload`, `/views`, `/views/new`, `/orgs`, `/share`, `/settings` + sub-pages `members`/`roles`/`audit`, `/login`, `/trash`, `/activity`, `/archive`); the everyday routes live under `app/(everyday)/`.
