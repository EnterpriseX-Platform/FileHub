...
export async function loadServerCtx(): Promise<{ cookieHeader: string; role: string | null; name: string | null }> {
  const cookieHeader = (await cookies()).getAll().map((c) => `${c.name}=${c.value}`).join("; ");
  const meRes = await fetch(`${BACKEND}/fh/api/auth/me`, {
    headers: cookieHeader ? { cookie: cookieHeader } : undefined,
    cache: "no-store",
  }).catch(() => null);
  const me: { role?: string; display_name?: string } | null = meRes && meRes.ok ? await meRes.json() : null;
  return { cookieHeader, role: me?.role ?? null, name: me?.display_name ?? null };
}
