// Deploy job: deploy-filehub-api-to-kube — modeled on
// jenkins-templates/deploy-neb-bpp-service-to-kube.groovy.
// Container name matches the Deployment (neb-filehub-api), see
// infra/sit-bb/k8s/deployment-neb-filehub-api.yaml.
pipeline {
    agent any

    environment {
        KUBECONFIG_CREDENTIAL_ID = "${params.KUBECONFIG_CREDENTIAL_ID}"
        DEPLOYMENT = 'neb-filehub-api'
    }

    parameters {
        string(name: 'APP_NAME', defaultValue: 'neb-filehub-api', description: 'Application name to deploy')
        string(name: 'DOCKER_IMAGE', defaultValue: 'avalantglobal/filehub-api:latest', description: 'Docker image to deploy')
            ...
            steps {
                withCredentials([file(credentialsId: "${KUBECONFIG_CREDENTIAL_ID}", variable: 'KUBECONFIG')]) {
                    ...
                    kubectl -n ${params.KUBE_NAMESPACE} set image deployment/${DEPLOYMENT} \
                        neb-filehub-api=${params.DOCKER_IMAGE}
                    """
                }
            }
        }
